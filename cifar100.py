...
def main():
    ...
    parser.add_argument('--net_name', type=str, default='mobilenetv2',
    ...
    parser.add_argument('--update_frequency', type=int, default=400,
    ...
    if args.train:
        ...
        for task_id in range(args.num_tasks):
            ...
            optimizer, lr_scheduler = set_optimizer(args.optimizer, args.lr, args.momentum, args.l2, args.epochs, net)

            if args.sparse:
                if task_id != 0:
                    net.load_state_dict(torch.load('./models/{0}_{1}_DENSE.pt'.format(args.dataset, args.net_name)))
                    if args.isolate:
                        used_params = freeze_used_params(used_params, args.dataset, task_id, args.net_name)
                death_rate_decay = DeathCosineDecay(args.death_rate, len(train_loader) * args.epochs)
                mask = Masking(args.init, args.density, args.death, args.growth,
                               args.death_rate, args.epochs, args.update_frequency, args.death_growth_ratio,
            ...
            for epoch in range(args.epochs):
                train_loss, train_acc = train(net, optimizer, task_id, train_loader,
                                              args.num_classes_per_task, args.regularize, args.isolate,
            ...
            if args.sparse:
                net.load_state_dict(torch.load('./models/{0}_{1}.pt'.format(args.dataset, args.net_name)))
                if task_id == 0: NET = copy.deepcopy(net)
                else:
                    for (name, param), (old_name, old_param) in zip(net.named_parameters(), NET.named_parameters()):
                        param.data[param == 0] = old_param.data[param == 0]
                        NET = copy.deepcopy(net)
                torch.save(NET.state_dict(), './models/{0}_{1}_DENSE.pt'.format(args.dataset, args.net_name))

            test_loader = get_loader(test_dataset, task_id=task_id, batch=128)
            masked_net = set_mask(args.dataset, args.net_name, NET, task_id)
            _, _, test_loss, test_accuracy = evaluate(masked_net, device, test_loader, args.num_classes_per_task, task_id)
    ...
    if args.test:
        ...
        for task_id in range(args.num_tasks):
            for t in range(args.num_tasks):
                test_loader = get_loader(test_dataset, task_id=t, batch=128)
                if args.sparse:
                    if task_id >= t:
                        ...
                        masked_net = set_mask(args.dataset, args.net_name, NET, task_id)
                        _, _, test_loss, test_accuracy = evaluate(masked_net, device, test_loader,
                                                                  args.num_classes_per_task, task_id, t,
                                                                  is_fwt=True)
